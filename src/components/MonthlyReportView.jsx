...
import { Printer, TrendingUp, TrendingDown, Users, IndianRupee, Package, Droplet, AlertCircle, Lightbulb, ChevronDown, ChevronUp, UserX } from "lucide-react";
...
export default function MonthlyReportView({ patients, entries, activityEntries, expenses }) {
  const now = new Date();
  ...
  // ---- Income ----
  // `entries` is cash-basis (dated to when payment was actually collected) —
  // used for money figures. `activityEntries` is accrual-basis (dated to the
  // visit itself) — used for patient counts and MR commission, which are tied
  // to the treatment happening rather than to when the bill got settled.
  function incomeStats(s, e) {
    let income = 0, cash = 0, upi = 0, card = 0;
    entries.forEach((en) => {
  ...
  const lostThisMonth = patients.filter((p) => {
    if (p.status !== "lost" || !p.lost_at) return false;
    const t = new Date(p.lost_at).getTime();
  ...
  if (lostThisMonth.length > 0) {
    advice.push(`${lostThisMonth.length} patient${lostThisMonth.length !== 1 ? "s" : ""} were marked lost this month — see the Lost Patients section below.`);
  }
  if (advice.length === 0) {
    ...
    <div>
      ...
      <div className="report-printable">
        ...
        <ReportSection title="Income & Profit" icon={<IndianRupee size={13} />}>
          <Row label="Total income (cash collected this month)" value={`₹${thisStats.income}`} />
          <Row label="MR commission (visits this month)" value={`₹${thisStats.mr}`} />
        ...
        {/* Lost patients — tucked away, collapsed by default */}
        <div className="bg-white rounded-2xl p-4 shadow-sm mb-3">
          <button
            onClick={() => setShowLostDetails((s) => !s)}
          ...
          >
            <div className="flex items-center gap-1.5">
              ...
              <p className="text-xs font-semibold" style={{ color: "#0A5C54" }}>
                Lost Patients ({lostThisMonth.length})
              </p>
            </div>
            {showLostDetails ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
          </button>
          <p className="hidden print:block text-xs font-semibold mb-2" style={{ color: "#0A5C54" }}>
            Lost Patients ({lostThisMonth.length})
          </p>
          {(showLostDetails) && (
            <div className="mt-2.5">
              {lostThisMonth.length === 0 ? (
                <p className="text-xs" style={{ color: "#0A5C5499" }}>No patients marked lost this month.</p>
              ) : (
                lostThisMonth.map((p) => {
                  const lastVisit = [...(p.visits || [])].sort((a, b) => b.ts - a.ts)[0];
                  return (
                    <div key={p.id} className="text-xs py-1.5 border-b" style={{ borderColor: "#14B8A61A" }}>
                      <div className="flex items-center justify-between">
                        <span className="font-medium" style={{ color: "#0A5C54" }}>{p.name}</span>
                        <span style={{ color: "#0A5C5499" }}>{new Date(p.lost_at).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}</span>
                      </div>
                      {lastVisit && (
                        <p style={{ color: "#0A5C5499" }}>Last visit: {lastVisit.complaint} — {new Date(lastVisit.ts).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}</p>
                      )}
                    </div>
                  );
                })
