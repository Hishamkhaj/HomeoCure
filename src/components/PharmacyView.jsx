...
import { Plus, X, Minus, AlertTriangle, ChevronLeft, Package } from "lucide-react";
...
export default function PharmacyView() {
  ...
  const [productForm, setProductForm] = useState({ name: "", price: "", unit_label: "", stock: "" });
  ...
  async function addProduct(e) {
    ...
    if (!error) {
      ...
      setProductForm({ name: "", price: "", unit_label: "", stock: "" });
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 mb-4 -mx-4 px-4">
        {categories.map((c) => (
          <button
            ...
            className="shrink-0 px-3.5 py-1.5 rounded-full text-xs font-medium whitespace-nowrap"
            style={{
                ...
                <div key={p.id} className="bg-white rounded-xl p-3.5 shadow-sm">
                  <div className="flex items-start justify-between mb-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs" style={{ color: "#0A5C5499" }}>
                        {p.unit_label ? `${p.unit_label} · ` : ""}₹{p.price}
                      </p>
                    </div>
                    <div
                      className="text-xs font-semibold px-2 py-0.5 rounded-full shrink-0"
                      style={{
                        background: low ? "#DC26261A" : "#148A7A1A",
                        color: low ? "#DC2626" : "#0A5C54",
                      }}
                    >
                      {p.stock} in stock
                    </div>
