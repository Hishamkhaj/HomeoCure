...
export default function TodayView({ patients, onSelect }) {
  ...
  // Part B: expected income from new/walk-in patients — typical amount for this weekday,
  // counting ONLY each patient's first-ever visit so it never overlaps with Part A.
  const firstVisitTsByPatient = {};
  patients.forEach((p) => {
    const visits = p.visits || [];
    if (visits.length > 0) firstVisitTsByPatient[p.id] = Math.min(...visits.map((v) => v.ts));
  });
  const newPatientDailyIncome = {};
  patients.forEach((p) => {
    (p.visits || []).forEach((v) => {
      if (v.ts === firstVisitTsByPatient[p.id]) {
        const key = dateStrFromTs(v.ts);
        newPatientDailyIncome[key] = (newPatientDailyIncome[key] || 0) + (v.cost || 0);
      }
    });
  });
  const newIncomeSamples = Object.entries(newPatientDailyIncome)
    .filter(([dateStr]) => dateStr !== todayStr)
    .filter(([dateStr]) => new Date(dateStr + "T12:00:00").getDay() === now.getDay())
    .map(([, income]) => income);
  const hasNewIncomeHistory = newIncomeSamples.length >= 3;
  const expectedNewPatientIncome = hasNewIncomeHistory
    ? Math.round(newIncomeSamples.reduce((a, b) => a + b, 0) / newIncomeSamples.length)
    : 0;

  const totalExpectedIncome = expectedFollowUpIncome + expectedNewPatientIncome;
  ...
  // ---- Today's income ----
  let patientIncomeToday = 0, mrToday = 0, cashAmt = 0, upiAmt = 0, cardAmt = 0, cashCount = 0, upiCount = 0, cardCount = 0;
  visitsToday.forEach(({ visit: v }) => {
    patientIncomeToday += v.cost || 0;
  ...
  directSalesToday.forEach((d) => {
    if (d.payment_mode === "upi") upiAmt += d.cost;
    else if (d.payment_mode === "card") cardAmt += d.cost;
    else cashAmt += d.cost;
  });
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm mb-3">
        <div className="flex items-center gap-1.5 mb-2.5" style={{ color: "#148A7A" }}>
          <Wallet size={13} />
          <p className="text-xs font-semibold" style={{ color: "#0A5C54" }}>Expected income today</p>
        </div>
        <div className="flex items-center justify-between text-xs py-1">
          <span style={{ color: "#0A5C5499" }}>
            From {exactDueToday.length} follow-up{exactDueToday.length !== 1 ? "s" : ""} due today
        ...
        <div className="flex items-center justify-between text-xs pb-2">
          <span></span>
          <span className="font-semibold" style={{ color: "#0A5C54" }}>₹{expectedFollowUpIncome}</span>
        </div>
        <div className="flex items-center justify-between text-xs py-1">
          <span style={{ color: "#0A5C5499" }}>
            From new / walk-in patients {hasNewIncomeHistory ? `(typical ${weekday})` : "(not enough history yet)"}
          </span>
        </div>
        <div className="flex items-center justify-between text-xs pb-2">
          <span></span>
          <span className="font-semibold" style={{ color: "#0A5C54" }}>₹{expectedNewPatientIncome}</span>
        </div>
        <div className="flex items-center justify-between pt-2 border-t" style={{ borderColor: "#14B8A655" }}>
          <span className="text-xs font-semibold" style={{ color: "#0A5C54" }}>Total expected</span>
          <span className="text-lg font-bold" style={{ color: "#148A7A" }}>₹{totalExpectedIncome}</span>
        </div>
      </div>
