...
export default function TodayView({ patients, onSelect }) {
  ...
  // ---- Pharmacy alerts ----
  const lowStockList = pharmacyProducts.filter((p) => {
  ...
  const refillsDueList = pharmacyProducts.filter(
    (p) => p.tracking_type === "volume" && refillEnabledCategoryIds.has(p.category_id) && (p.remaining_ml ?? 0) <= (p.low_volume_threshold_ml ?? 50)
  );
