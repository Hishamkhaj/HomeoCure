...
import { Users, IndianRupee, TrendingUp, TrendingDown, Minus, Wallet } from "lucide-react";
...
// Merge patient visits + standalone pharmacy sales (walk-in items not tied to a patient) into one flat list
function flattenEntries(patients, directSales) {
  const list = [];
  patients.forEach((p) => {
    (p.visits || []).forEach((v) => {
      list.push({ ts: v.ts, cost: v.cost || 0, mr: v.mr_commission || 0, payment_mode: v.payment_mode || "cash", name: p.name });
    });
  });
  directSales.forEach((s) => {
    list.push({ ts: s.ts, cost: s.cost, mr: 0, payment_mode: "cash", name: s.name });
  });
  return list;
}

function visitsInRange(entries, start, end) {
  return entries
    .filter((e) => e.ts >= start.getTime() && e.ts < end.getTime())
    .sort((a, b) => b.ts - a.ts);
}
...
function PatientBreakdown({ visits }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-4 shadow-sm mb-4">
      <p className="text-xs font-semibold mb-3" style={{ color: "#0A5C54" }}>Patient payments</p>
      {visits.length === 0 ? (
        <p className="text-xs text-center py-4" style={{ color: "#0A5C5466" }}>No payments in this period.</p>
      ) : (
      ...
      <div className="flex items-center justify-between mt-3 pt-3 border-t" style={{ borderColor: "#14B8A655" }}>
        <span className="text-xs font-semibold" style={{ color: "#0A5C54" }}>Grand total</span>
...
export default function IncomeView({ patients }) {
  ...
  const todayVisits = visitsInRange(entries, todayStart, todayEnd);
  ...
  const thisWeekVisits = visitsInRange(entries, weekStart, weekEnd);
  ...
  const thisMonthVisits = visitsInRange(entries, monthStart, monthEnd);
  ...
  const active =
    subTab === "daily"
      ? { current: todayStats, previous: yesterdayStats, periodLabel: "Today", visits: todayVisits }
      : subTab === "weekly"
      ? { current: thisWeekStats, previous: lastWeekStats, periodLabel: "This week", visits: thisWeekVisits }
      : { current: thisMonthStats, previous: lastMonthStats, periodLabel: "This month", visits: thisMonthVisits };
  ...
  return (
    <div>
      ...
      <PatientBreakdown visits={active.visits} />
