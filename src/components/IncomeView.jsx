...
function visitsInRange(patients, start, end) {
  const list = [];
  patients.forEach((p) => {
    (p.visits || []).forEach((v) => {
      if (v.ts >= start.getTime() && v.ts < end.getTime()) {
        list.push({ name: p.name, cost: v.cost || 0, payment_mode: v.payment_mode, ts: v.ts });
      }
    });
  });
...
export default function IncomeView({ patients }) {
  ...
  const todayVisits = visitsInRange(patients, todayStart, todayEnd);
  ...
  const thisWeekVisits = visitsInRange(patients, weekStart, weekEnd);
  ...
  const thisMonthVisits = visitsInRange(patients, monthStart, monthEnd);
