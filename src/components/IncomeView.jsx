...
import { Users, IndianRupee, TrendingUp, TrendingDown, Minus, Wallet, Package, Calendar } from "lucide-react";
...
// their paid_amount as received on the visit date, so existing data keeps working.
...
// Cash-basis: one row per actual payment received, dated to when it was collected.
// Used for all money figures (income, payment-mode breakdown, trend).
function buildIncomeEntries(patients, directSales) {
  const list = [];
  patients.forEach((p) => {
    (p.visits || []).forEach((v) => {
      getPaymentEvents(v).forEach((e) => {
        list.push({ ts: e.ts, cost: e.amount, payment_mode: e.mode, name: p.name, type: "patient" });
      });
    });
  });
  directSales.forEach((s) => {
    list.push({ ts: s.ts, cost: s.cost, payment_mode: s.payment_mode || "cash", name: s.name, type: "product" });
  });
  return list;
}

// Accrual-basis: one row per patient visit, dated to the visit itself. Used for
// "how many patients were seen" and MR commission, which are tied to the
// treatment happening, not to when the money was collected.
function buildActivityEntries(patients) {
  const list = [];
  patients.forEach((p) => {
    (p.visits || []).forEach((v) => {
      list.push({ ts: v.ts, mr: v.mr_commission || 0 });
    });
  });
  return list;
}
...
function statsForRange(incomeEntries, activityEntries, start, end) {
  let income = 0, cash = 0, upi = 0, card = 0, patientIncome = 0, productIncome = 0;
  ...
  return { count: patientCount, patientCount, income, mr, profit: income - mr, cash, upi, card, patientIncome, productIncome };
}

function dailySeries(incomeEntries, start, days) {
  const map = {};
...
export default function IncomeView({ patients }) {
  ...
  const [selectedDateStr, setSelectedDateStr] = useState(() => dateStrFromTs(Date.now()));
  const now = new Date();
  ...
  const incomeEntries = buildIncomeEntries(patients, directSales);
  const activityEntries = buildActivityEntries(patients);

  // ---- Daily (any date the user picks, defaults to today) ----
  const selectedDate = new Date(selectedDateStr + "T12:00:00");
  const dayStart = startOfDay(selectedDate);
  const dayEnd = addDays(dayStart, 1);
  const prevDayStart = addDays(dayStart, -1);
  const dayStats = statsForRange(incomeEntries, activityEntries, dayStart, dayEnd);
  const prevDayStats = statsForRange(incomeEntries, activityEntries, prevDayStart, dayStart);
  const last7Trend = dailySeries(incomeEntries, addDays(dayStart, -6), 7);
  const dayIncomeEntries = inRange(incomeEntries, dayStart, dayEnd);

  // ---- Weekly (always current week) ----
  const { start: weekStart, end: weekEnd } = weekRange(now);
  const prevWeekStart = addDays(weekStart, -7);
  const thisWeekStats = statsForRange(incomeEntries, activityEntries, weekStart, weekEnd);
  const lastWeekStats = statsForRange(incomeEntries, activityEntries, prevWeekStart, weekStart);
  const thisWeekIncomeEntries = inRange(incomeEntries, weekStart, weekEnd);

  // ---- Monthly (always current month) ----
  ...
  const thisMonthStats = statsForRange(incomeEntries, activityEntries, monthStart, monthEnd);
  const lastMonthStats = statsForRange(incomeEntries, activityEntries, prevMonthStart, prevMonthEnd);
  const thisMonthIncomeEntries = inRange(incomeEntries, monthStart, monthEnd);
  ...
  const isToday = selectedDateStr === dateStrFromTs(Date.now());
  const active =
    subTab === "daily"
      ? { current: dayStats, previous: prevDayStats, periodLabel: isToday ? "Today" : selectedDate.toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" }), entries: dayIncomeEntries }
      : subTab === "weekly"
      ? { current: thisWeekStats, previous: lastWeekStats, periodLabel: "This week", entries: thisWeekIncomeEntries }
      : { current: thisMonthStats, previous: lastMonthStats, periodLabel: "This month", entries: thisMonthIncomeEntries };
  ...
  return (
    <div>
      ...
      {subTab === "expenses" ? (
        ...
        <MonthlyReportView patients={patients} entries={incomeEntries} activityEntries={activityEntries} expenses={expenses} />
      ) : (
        <>
      {subTab === "daily" && (
        <div className="flex items-center gap-2 bg-white rounded-xl px-3 py-2.5 shadow-sm mb-3" style={{ border: "1px solid #14B8A633" }}>
          <Calendar size={16} color="#148A7A" />
          <input
            type="date"
            value={selectedDateStr}
            max={dateStrFromTs(Date.now())}
