...
export default function AnalyticsView() {
  ...
  // Classify by the product's CURRENT tracking_type (not by which field happens to
  // be set on each sale row) so a product that was reconfigured from pieces->ml
  // (or vice versa) doesn't show up split across both tables.
  const productById = {};
  products.forEach((p) => {
    productById[p.id] = p;
  });
  ...
  function unitRecommendation(rec) {
    ...
    if (!prod || prod.tracking_type === "volume") return null; // now a measured product — skip from pieces list
  ...
  function volRecommendation(rec) {
    const prod = products.find((p) => p.id === rec.product_id);
    if (!prod || prod.tracking_type !== "volume") return null; // now a pieces product — skip from measured list
