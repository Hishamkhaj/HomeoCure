...
import PatientsAnalyticsView from "./components/PatientsAnalyticsView";
...
export default function App() {
  ...
  function buildVisit(form) {
    ...
    return {
      ...
      // Each entry records WHEN money was actually collected, so income can be
      // reported on the day it was received rather than the treatment date.
      payment_log: paidAmount > 0 ? [{ date: visitDateStr, amount: paidAmount, mode: form.payment_mode || "cash" }] : [],
    };
  }
  ...
  async function handleEditVisit(originalTs, form) {
    const originalVisit = (selected.visits || []).find((v) => v.ts === originalTs);
    // Editing a visit shouldn't wipe out its recorded payment history — that's only
    // ever added to via the dedicated "Record Payment" flow. A changed paid_amount
    // here is treated as a manual correction, not a new payment event.
    const updatedVisit = { ...buildVisit(form), payment_log: originalVisit?.payment_log || [] };
  ...
  // Applies `amountToApply` against a patient's oldest unpaid visits first (FIFO),
  // recording each payment with today's date so income lands on the day it's
  // actually collected. `mode` is the payment mode used for this transaction.
  async function handleRecordPayment(patient, amountToApply, mode) {
    const nowStr = (() => {
      const d = new Date();
      return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
    })();
    let remaining = amountToApply;
    const visitsSorted = [...(patient.visits || [])].sort((a, b) => a.ts - b.ts);
    const updatedVisits = visitsSorted.map((v) => {
      if (remaining <= 0) return v;
      const due = Math.max(0, (v.cost || 0) - (v.paid_amount ?? v.cost ?? 0));
      if (due <= 0) return v;
      const applyHere = Math.min(due, remaining);
      remaining -= applyHere;
      const newPaid = (v.paid_amount ?? v.cost ?? 0) + applyHere;
      const newLog = [...(v.payment_log || []), { date: nowStr, amount: applyHere, mode: mode || "cash" }];
      return { ...v, paid_amount: newPaid, payment_log: newLog };
    });
  ...
  const MORE_ITEMS = [
    ...
    { key: "patientAnalytics", label: "Patient Stats", icon: Users, color: "#8B5CF6" },
